...
if current_cell_flow_direction > 0
    ...
    % cell so the next cell may point back to this current cell.
    direction_from_next_cell_to_current_cell = mod(current_cell_flow_direction - pi, 2*pi);
    
    % Find indices of next_cell by finding current_cell indices and current
    % direction, then finding the indices of the cell to which the current cell
    % is directed. 
    ...
    % If it is a diagonal(ie x and y are both 1), the distance is sqrt(2)
