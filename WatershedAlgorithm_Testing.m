...
%% Flow Accumulation (Drainage Included)
tic_flow_acc_drain = tic;
%net_flow_accumulation = netFlowAccumulation(flow_direction, drainage);
flow_acc_drain_time = toc(tic_flow_acc_drain);

%% Identify Pits, Compute Matrix/Map with Pit ID for each cell
pits_tic = tic;
[pits, pit_data] = Pits(dem, drainage, flow_direction, cellsize, intensity);
...
color_map = [0 0 0; cell2mat(pit_data(:, 15))];
...
%% Fill Pits
fill_tic = tic;
[fill_dem, puddle_dem, fill_flow_direction, fill_pits, sort_pit_data] = fillPits(dem, flow_direction, pits, pit_data, rain_duration, rain_depth, intensity, cellsize, color_map);
